...
export const withStreamSpan =
  ...
  <R, E, A>(
    stream: S.Stream<R & HasSpan, E, A>
  ): S.Stream<R & OT.HasTracer, E, A> =>
    pipe(
      ...
      S.chain((r) =>
        pipe(
          M.gen(function* ($) {
                ...
                pipe(
                  T.succeedWith(() => {
                    ...
                    if (options?.root !== true && O.isSome(maybeSpan)) {
                      const ctx = OTApi.trace.setSpan(
                        OTApi.context.active(),
                        maybeSpan.value.span
                      );
                      return tracer.startSpan(name, options, ctx);
                    }
            ...
            return pipe(
              stream,
              S.provideAll({ ...r, ...OT.Span.has(new OT.SpanImpl(span)) })
            );
          }),
          S.unwrapManaged
        )
