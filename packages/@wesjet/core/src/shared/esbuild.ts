      ...
      return T.gen(function* ($) {
        ...
        const buildWatchPlugin: esbuild.Plugin = {
          ...
          setup(build) {
            ...
            build.onEnd((result) => {
              runtime.runFiber(
                OT.addEvent('esbuild-build-result', { result: JSON.stringify(result) }),
              )
              ...
              if (isFirstBuild) {
                ...
                if (result.errors.length > 0) {
                  runtime.runFiber(
                    H.publish_(
                      fsEventsHub,
                      Ex.succeed(E.left(new KnownEsbuildError({ error: result.errors }))),
                    ),
                  )
                } else {
...
export const make = (
  ...
  T.gen(function* ($) {
    ...
    const hub = yield* $(
      H.makeUnbounded<Ex.Exit<never, E.Either<EsbuildError, esbuild.BuildResult>>>(),
    )
  ...
  pipe(
    ...
    T.chain((result) =>
      result.errors.length > 0
        ? T.fail(new KnownEsbuildError({ error: result.errors }))
        : T.succeed(result),
    ),
