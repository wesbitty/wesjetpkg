    ...
    T.chain((buildContext) =>
      T.tryPromise(async () => {
        if (O.isSome(buildContext)) {
          ...
          throw new Error(
            `This should never happen. Esbuild build context is not set.`
          );
        }
      })
    ),
    ...
    T.suspend(() => {
      ...
      return T.gen(function* ($) {
        ...
        const buildWatchPlugin: esbuild.Plugin = {
          name: "wesjet-watch-plugin",
          setup(build) {
            let isFirstBuild = false;

            build.onEnd((result) => {
              runtime.runFiber(
                OT.addEvent("esbuild-build-result", {
                ...
                if (result.errors.length > 0) {
                  runtime.runFiber(
                    H.publish_(
                      fsEventsHub,
                      Ex.succeed(
                        E.left(new KnownEsbuildError({ error: result.errors }))
                      )
                    )
                  );
                } else {
                  runtime.runFiber(
                    H.publish_(fsEventsHub, Ex.succeed(E.right(result!)))
                  );
                }
              }
            });
...
export const make = (
  ...
  T.gen(function* ($) {
    const initialBuildResult = yield* $(
    ...
    const buildContext = yield* $(
      Ref.makeRef<O.Option<esbuild.BuildContext>>(O.none)
    );

    return new ConcreteEsbuildWatcher(
      initialBuildResult,
      buildContext,
      buildOptions,
      hub
