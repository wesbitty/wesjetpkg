/**
 * @fileoverview  Enforce that types will not to be used
 * @author dimejiSR
 */
'use strict'
...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      extraDescription: [util.tslintRule('ts-ban-types')],
      category: 'TypeScript',
      url: util.metaDocsUrl('ts-ban-types'),
      recommended: 'error',
    },
    fixable: 'code',
    messages: {
  ...
  create(context) {
    const banedTypes = util.applyDefault(defaultOptions, context.options)[0].types
        ...
        if (node.parent && node.parent.type !== 'TSQualifiedName') {
          if (node.name in banedTypes) {
            ...
            context.report({
              ...
              fix: fixWith !== null && ((fixer) => fixer.replaceText(node, fixWith)),
            })
          }
        }
      },
