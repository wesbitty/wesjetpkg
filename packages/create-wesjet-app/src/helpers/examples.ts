...
export function existsInRepo(nameOrUrl: string): Promise<boolean> {
  try {
    ...
    return isUrlOk(
      `https://api.github.com/repos/wesbitty/wesjet/contents/examples/${encodeURIComponent(
        nameOrUrl,
      )}`,
    )
  }
...
async function downloadTar(url: string) {
  const tempFile = join(tmpdir(), `wesjet-cna-example.temp-${Date.now()}`)
...
export async function downloadAndExtractExample(root: string, name: string) {
  ...
  await tar.x({
    ...
    filter: (p) => p.includes(`wesjet-automated/examples/${name}/`),
  })
